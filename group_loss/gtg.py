...
class GTG(nn.Module):
    ...
    def forward(self, model_embed, num_points, labs, L, U, probs=None, classes_to_use=None):
        W = self._get_W(model_embed)
        ...
        ps = dynamics.dynamics(W, ps, self.tol, self.max_iter, self.mode)  # ps is x as mentioned in group loss paper
        return ps, W